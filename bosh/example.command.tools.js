                    ...
                    for (var i = 0; i < stackedParts.length; i++) {
                        ...
                        if (suitableRedirectors.length) {
                            ...
                            if (suitableHandlers.length == 0){
                                      var localvar=$.trim(consoleInput.split(/=(.+)/)[0]);
                                      var pure_command=$.trim(consoleInput.split(/=(.+)/)[1]);                            
                                      p = commandLineSplitter.split(pure_command); 
                                      suitableHandlers = me.handlers.filter(function(item){return p.length && item.command == p[0].toLowerCase()});
                                      if (suitableHandlers.length == 0 ){
                                             throw new Error("There is no suitable handler for that command.");
                                      }else{
                                             tempSession.localvar = localvar;
                                      }
                            }
                            var h = suitableHandlers[0];

                            p[0] = tempSession;
                            result = h.handle.apply(h, p);
                        }
                    }
