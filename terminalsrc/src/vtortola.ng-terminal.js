...
.provider('terminalConfiguration', function () {
    ...
    var config = function () {
        ...
        me.promptConfiguration = { end: '>', user: 'bosh', separator: '', path: '' };
...
//        $scope.$$phase || $scope.$apply();
    ...
    $scope.keypress= function (keyCode) {
        if ($scope.commandLine.length < 2400) {
            commandIndex = -1;
    ...
    $scope.execute = function () {
        var command = $scope.cursor!='_' ? $scope.commandLine + $scope.cursor + $scope.commandEND : $scope.commandLine;
        command = cleanNonPrintableCharacters(command);
    ...
    $scope.left = function() {
         if ($scope.commandLine) {
            $scope.commandEND = $scope.cursor + $scope.commandEND;
            $scope.cursor = $scope.commandLine.slice(-1);
    ...
    $scope.right = function() {
            if($scope.commandEND){
                  $scope.commandLine = $scope.commandLine + $scope.cursor;
                  $scope.cursor = $scope.commandEND[0];
                  $scope.commandEND = $scope.commandEND.substring(1, $scope.commandEND.length);
                  $scope.$apply();
            }
    }
...
.directive('terminal', function ($document) {
    return {
        ...
        transclude: true,
        replace:true,
        template: "<section class='terminal' ng-paste='handlePaste($event)'><div class='terminal-viewport'><div class='terminal-results'></div><span class='terminal-prompt' ng-show='showPrompt'>{{prompt.text}}</span><span class='terminal-input'>{{commandLine}}</span><span ng-model='cursor' class='terminal-cursor'>{{cursor}}</span><span class='terminal-input'>{{commandEND}}</span><input type='text' ng-model='commandLine' class='terminal-target'/><input type='text' ng-model='cursor' class='terminal-target'/><input type='text' ng-model='commandEND' class='terminal-target'/></div><div ng-transclude></div></section>",
        compile: function compile(tElement, tAttrs, transclude) {
            return {
                ...
                post: function postLink(scope, element, attrs, controller) { 
                        ...
                        consoleView.on('dblclick', function () {
                               target[0].focus();
